...
x_pos <- min(data[[x_col]]) + (max(data[[x_col]]) - min(data[[x_col]])) * 0.55
y_pos <- min(data[[y_col]]) + (max(data[[y_col]]) - min(data[[y_col]])) * 0.15

# Use aes() with .data[[...]] instead of deprecated aes_string()
...
ggsave("regression_plot_r.png", plot, width = 8, height = 6, dpi = 150)
cat("Saved plot: regression_plot_r.png\n")
